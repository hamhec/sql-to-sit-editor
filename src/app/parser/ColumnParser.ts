...
export const StrToColumnObject = (str: string): Column => {
    ...
    const words = Helper.splitOnASKeyword(str);
    if(words.length != 3) {
        throw new NoAliasError(str);
    }
    col.name = Helper.removeUnnecessaryWhiteSpaces(words[0]);
    col.alias = Helper.removeUnnecessaryWhiteSpaces(words[2]);
